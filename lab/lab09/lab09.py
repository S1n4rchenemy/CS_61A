...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            return [[prev]] + inc_subseqs(s) if prev >= 0 else [[]]
        elif s[0] < prev:
            return subseq_helper(s[1:], prev) + inc_subseqs(s)
        else:
            #a = 
            #b = ______________________
            return insert_into_all(prev, inc_subseqs(s)) + inc_subseqs(s)
    return subseq_helper(s[1:], s[0] if s else -1)
