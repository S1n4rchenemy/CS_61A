...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def splits_helper(s_left, s_right, d):
        diff = sum(s_right) - sum(s_left)
        if abs(diff) <= d:
            count = 1
        else:
            count = 0
        ...
        if not len(s_left) == 0:
            index_remain = -1 * (len(s) - s.index(s_left[-1]) - 1)
        else:
            index_remain = -1 * len(s)

        if index_remain == 0:
            return count
        else:
            for i in range(index_remain, 0):
                next_left = s_left + [s[i]]
                next_right = s_right[: len(s_right) + i] + s_right[len(s_right) + i + 1 :]
                count += splits_helper(next_left, next_right, d)
            return count 
    
    return splits_helper([], s, d) // 2
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            possibilities.append((identity+match, cost))
        # Insert
        insert = "+[" + code_char + "]"
        match, cost = helper_align(skeleton_idx, code_idx+1)
        possibilities.append((insert+match, cost+1))
        # Delete
        delete = "-[" + skel_char + "]"
        match, cost = helper_align(skeleton_idx+1, code_idx)
        possibilities.append((delete+match, cost+1))
...
def filterl(lst, pred):
    ...
    return foldl(lst, filter, Link.empty)
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda link, n: Link(n, link), Link.empty)


def foldr(link, fn, z):
    """ Right fold
    >>> list = Link(3, Link(2, Link(1)))
    >>> foldr(list, sub, 0) # (((0 - 1) - 2) - 3)
    -6
    >>> foldr(list, add, 0) # (((0 + 1) + 2) + 3)
    6
    >>> foldr(list, mul, 1) # (((1 * 1) * 2) * 3)
    6
    """
    return foldl(reverse(link), fn, z)
