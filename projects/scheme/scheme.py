...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        fn_expressions = Pair(target.rest, expressions.rest) 
        env.define(fn_name, do_lambda_form(fn_expressions, env))
        return fn_name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if len(bindings.first.rest) == 1:
            value = scheme_eval(bindings.first.rest.first, env)
        else:
            value = scheme_eval(bindings.first.rest, env)
