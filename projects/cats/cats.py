...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0:
        ...
        check_list = [1 for x in range(len(typed_words)) if typed_words[x] == reference_words[x]]
        return (sum(check_list) / len(typed_words)) * 100
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        diff_result = min(valid_words, key = diff_func)
        if diff_function(user_word, diff_result, limit) > limit:
            return user_word 
        else:
            return diff_result
    # END PROBLEM 5
